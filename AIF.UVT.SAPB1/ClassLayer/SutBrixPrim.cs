...
{
    public class SutBrixPrim
    {
        [ItemAtt(AIFConn.SutBrixPrimUID)]
        public SAPbouiCOM.Form frmSutBrixPrim;
        ...
        {
            ConstVariables.oFnc.LoadSAPXML(AIFConn.SutBrixPrimXML, Assembly.GetExecutingAssembly().GetManifestResourceStream(AIFConn.SutBrixPrimXML));
            Functions.CreateUserOrSystemFormComponent<SutBrixPrim>(AIFConn.BrixPrim);
        ...
        public void InitForms()
        {
            try
            {
                frmSutBrixPrim.EnableMenu("1283", false);
                frmSutBrixPrim.EnableMenu("1284", false);
                frmSutBrixPrim.EnableMenu("1286", false);
            ...
            try
            {
                if (pVal.MenuUID == "AIFRGHTCLK_DeleteRow" && pVal.BeforeAction)
                {
                    ...
                    if (row != -1)
                    {
                        ...
                        if (frmSutBrixPrim.Mode == BoFormMode.fm_OK_MODE)
                        {
                            frmSutBrixPrim.Mode = BoFormMode.fm_UPDATE_MODE;
                        }
                    }
                }
                else if (pVal.MenuUID == "AIFRGHTCLK_AddRow" && pVal.BeforeAction)
                {
                    frmSutBrixPrim.DataSources.DBDataSources.Item(1).Clear();
                    oMatrix.AddRow();
