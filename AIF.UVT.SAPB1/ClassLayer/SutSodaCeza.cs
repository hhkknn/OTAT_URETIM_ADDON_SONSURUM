...
{
    public class SutSodaCeza
    {
        [ItemAtt(AIFConn.SutSodaCezaUID)]
        public SAPbouiCOM.Form frmSutSodaCeza;
        ...
        {
            ConstVariables.oFnc.LoadSAPXML(AIFConn.SutSodaCezaXML, Assembly.GetExecutingAssembly().GetManifestResourceStream(AIFConn.SutSodaCezaXML));
            Functions.CreateUserOrSystemFormComponent<SutSodaCeza>(AIFConn.SodaCz);
        ...
        public void InitForms()
        {
            try
            {
                frmSutSodaCeza.EnableMenu("1283", false);
                frmSutSodaCeza.EnableMenu("1284", false);
                frmSutSodaCeza.EnableMenu("1286", false);
            ...
            try
            {
                if (pVal.MenuUID == "AIFRGHTCLK_DeleteRow" && pVal.BeforeAction)
                {
                    ...
                    if (row != -1)
                    {
                        ...
                        if (frmSutSodaCeza.Mode == BoFormMode.fm_OK_MODE)
                        {
                            frmSutSodaCeza.Mode = BoFormMode.fm_UPDATE_MODE;
                        }
                    }
                }
                else if (pVal.MenuUID == "AIFRGHTCLK_AddRow" && pVal.BeforeAction)
                {
                    frmSutSodaCeza.DataSources.DBDataSources.Item(1).Clear();
                    oMatrix.AddRow();
