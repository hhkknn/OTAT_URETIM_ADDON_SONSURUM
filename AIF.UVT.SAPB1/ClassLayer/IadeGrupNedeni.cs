...
{
    public class IadeGrupNedeni
    {
        [ItemAtt(AIFConn.IadeGrupNedeniUID)]
        public SAPbouiCOM.Form frmIadeGrupNedeni;
        ...
        {
            ConstVariables.oFnc.LoadSAPXML(AIFConn.IadeGrupNedeniXML, Assembly.GetExecutingAssembly().GetManifestResourceStream(AIFConn.IadeGrupNedeniXML));
            Functions.CreateUserOrSystemFormComponent<IadeGrupNedeni>(AIFConn.IadeGrpNdn);
        ...
        public void InitForms()
        {
            try
            {
                frmIadeGrupNedeni.Freeze(true);

                frmIadeGrupNedeni.EnableMenu("1283", false);
                frmIadeGrupNedeni.EnableMenu("1284", false);
                frmIadeGrupNedeni.EnableMenu("1286", false);

                oDataTable = frmIadeGrupNedeni.DataSources.DataTables.Add("DATA");
                ...
                string sql = "Select * FROM \"@AIF_IADENEDENI\" ";
                ...
                if (ConstVariables.oRecordset.RecordCount > 0)
                {
                    try
                    {
                        frmIadeGrupNedeni.DataSources.DBDataSources.Item(0).Query();
                        oMatrix.LoadFromDataSource();
                        if (oMatrix.RowCount > 0)
                        {
                            frmIadeGrupNedeni.Mode = BoFormMode.fm_OK_MODE;
                        }
                    }
                    catch (Exception)
                    {
            ...
            {
                frmIadeGrupNedeni.Freeze(false);
            }
        ...
        public bool SAP_FormDataEvent(ref BusinessObjectInfo BusinessObjectInfo, ref bool BubbleEvent)
        {
            ...
            switch (BusinessObjectInfo.EventType)
            {
                ...
                case BoEventTypes.et_FORM_DATA_ADD:
                    ...
                    {
                        string sonsatir1 = frmIadeGrupNedeni.DataSources.DBDataSources.Item(0).GetValue("DocEntry", frmIadeGrupNedeni.DataSources.DBDataSources.Item(0).Size - 1);

                        if (sonsatir1 == "")
                        {
                            frmIadeGrupNedeni.DataSources.DBDataSources.Item(0).RemoveRecord(frmIadeGrupNedeni.DataSources.DBDataSources.Item(0).Size - 1);
                        }
                    }
                    break;
        ...
        public bool SAP_ItemEvent(string FormUID, ref ItemEvent pVal, ref bool BubbleEvent)
        {
            ...
            switch (pVal.EventType)
            {
                ...
                case BoEventTypes.et_ITEM_PRESSED:
                    if (pVal.ItemUID == "1" && !pVal.BeforeAction)
                    {
                        if (silinecekler.Count > 0)
                        {
                            foreach (var item in silinecekler)
                            {
                                if (item != "")
                                {
                                    ConstVariables.oRecordset.DoQuery("Delete from \"@AIF_IADENEDENI\" where \"DocEntry\" = '" + item + "'");
                                }
                            }
                        }
                    ...
                    //        //frmIadeGrupNedeni.DataSources.DBDataSources.Item("@AIF_IADENEDENI").Clear();

                    //        //oMatrix.AddRow();
                    //        //frmIadeGrupNedeni.Mode = BoFormMode.fm_UPDATE_MODE;
                ...
                case BoEventTypes.et_CLICK:
                    if (pVal.ItemUID == "1" && !pVal.BeforeAction)
                    {
                        ConstVariables.oRecordset.DoQuery("Select MAX(\"DocEntry\") from \"@AIF_IADENEDENI\"");
            ...
            try
            {
                if (pVal.MenuUID == "AIFRGHTCLK_DeleteRow" && pVal.BeforeAction)
                {
                    ...
                    if (row != -1)
                    {
                        ...
                        if (frmIadeGrupNedeni.Mode == BoFormMode.fm_OK_MODE)
                        {
                            frmIadeGrupNedeni.Mode = BoFormMode.fm_UPDATE_MODE;
                        }
                    }
                ...
                {
                    frmIadeGrupNedeni.DataSources.DBDataSources.Item(0).Clear();
                    oMatrix.AddRow();
                    frmIadeGrupNedeni.Mode = BoFormMode.fm_UPDATE_MODE;
