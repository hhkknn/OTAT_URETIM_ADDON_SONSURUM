...
{
    public class SutPhCeza
    {
        [ItemAtt(AIFConn.SutPhCezaUID)]
        public SAPbouiCOM.Form frmSutPhCeza;
        ...
        {
            ConstVariables.oFnc.LoadSAPXML(AIFConn.SutPhCezaXML, Assembly.GetExecutingAssembly().GetManifestResourceStream(AIFConn.SutPhCezaXML));
            Functions.CreateUserOrSystemFormComponent<SutPhCeza>(AIFConn.PhCeza);
        ...
        public void InitForms()
        {
            try
            {
                frmSutPhCeza.EnableMenu("1283", false);
                frmSutPhCeza.EnableMenu("1284", false);
                frmSutPhCeza.EnableMenu("1286", false);
            ...
            try
            {
                if (pVal.MenuUID == "AIFRGHTCLK_DeleteRow" && pVal.BeforeAction)
                {
                    ...
                    if (row != -1)
                    {
                        ...
                        if (frmSutPhCeza.Mode == BoFormMode.fm_OK_MODE)
                        {
                            frmSutPhCeza.Mode = BoFormMode.fm_UPDATE_MODE;
                        }
                    }
                }
                else if (pVal.MenuUID == "AIFRGHTCLK_AddRow" && pVal.BeforeAction)
                {
                    frmSutPhCeza.DataSources.DBDataSources.Item(1).Clear();
                    oMatrix.AddRow();
