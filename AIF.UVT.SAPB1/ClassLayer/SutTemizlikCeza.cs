...
{
    public class SutTemizlikCeza
    {
        [ItemAtt(AIFConn.SutTemizlikCezaUID)]
        public SAPbouiCOM.Form frmSutTemizlikCeza;
        ...
        {
            ConstVariables.oFnc.LoadSAPXML(AIFConn.SutTemizlikCezaXML, Assembly.GetExecutingAssembly().GetManifestResourceStream(AIFConn.SutTemizlikCezaXML));
            Functions.CreateUserOrSystemFormComponent<SutTemizlikCeza>(AIFConn.TmzlkCeza);
        ...
        public void InitForms()
        {
            try
            {
                frmSutTemizlikCeza.EnableMenu("1283", false);
                frmSutTemizlikCeza.EnableMenu("1284", false);
                frmSutTemizlikCeza.EnableMenu("1286", false);
            ...
            try
            {
                if (pVal.MenuUID == "AIFRGHTCLK_DeleteRow" && pVal.BeforeAction)
                {
                    ...
                    if (row != -1)
                    {
                        ...
                        if (frmSutTemizlikCeza.Mode == BoFormMode.fm_OK_MODE)
                        {
                            frmSutTemizlikCeza.Mode = BoFormMode.fm_UPDATE_MODE;
                        }
                    }
                ...
                {
                    frmSutTemizlikCeza.DataSources.DBDataSources.Item(1).Clear();
                    oMatrix.AddRow();
                    oMatrix.Columns.Item("Col_0").Cells.Item(oMatrix.RowCount).Click();
