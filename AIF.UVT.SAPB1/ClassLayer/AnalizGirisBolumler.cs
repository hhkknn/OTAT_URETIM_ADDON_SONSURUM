...
{
    public class AnalizGirisBolumler
    {
        [ItemAtt(AIFConn.AnalizGirisBolumlerUID)]
        public SAPbouiCOM.Form frmAnalizGirisBolumler;
        ...
        {
            ConstVariables.oFnc.LoadSAPXML(AIFConn.AnalizGirisBolumlerXML, Assembly.GetExecutingAssembly().GetManifestResourceStream(AIFConn.AnalizGirisBolumlerXML));
            Functions.CreateUserOrSystemFormComponent<AnalizGirisBolumler>(AIFConn.AnlzBolum);
        ...
        public void InitForms()
        {
            try
            {
                frmAnalizGirisBolumler.Freeze(true);

                frmAnalizGirisBolumler.EnableMenu("1283", false);
                frmAnalizGirisBolumler.EnableMenu("1284", false);
                frmAnalizGirisBolumler.EnableMenu("1286", false);

                oDataTable = frmAnalizGirisBolumler.DataSources.DataTables.Add("DATA");
                ...
                string sql = "Select * FROM \"@AIF_ANLZGIRISBOLUM\" ";
                ...
                if (ConstVariables.oRecordset.RecordCount > 0)
                {
                    try
                    {
                        frmAnalizGirisBolumler.DataSources.DBDataSources.Item(0).Query();
                        oMatrix.LoadFromDataSource();
                        if (oMatrix.RowCount > 0)
                        {
                            frmAnalizGirisBolumler.Mode = BoFormMode.fm_OK_MODE;
                        }
                    }
                    catch (Exception)
                    {
            ...
            {
                frmAnalizGirisBolumler.Freeze(false);
            }
        ...
        public bool SAP_FormDataEvent(ref BusinessObjectInfo BusinessObjectInfo, ref bool BubbleEvent)
        {
            ...
            switch (BusinessObjectInfo.EventType)
            {
                ...
                case BoEventTypes.et_FORM_DATA_ADD:
                    ...
                    {
                        string sonsatir1 = frmAnalizGirisBolumler.DataSources.DBDataSources.Item(0).GetValue("DocEntry", frmAnalizGirisBolumler.DataSources.DBDataSources.Item(0).Size - 1);

                        if (sonsatir1 == "")
                        {
                            frmAnalizGirisBolumler.DataSources.DBDataSources.Item(0).RemoveRecord(frmAnalizGirisBolumler.DataSources.DBDataSources.Item(0).Size - 1);
                        }
                    }
                    break;
        ...
        public bool SAP_ItemEvent(string FormUID, ref ItemEvent pVal, ref bool BubbleEvent)
        {
            ...
            switch (pVal.EventType)
            {
                ...
                case BoEventTypes.et_ITEM_PRESSED:
                    if (pVal.ItemUID == "1" && !pVal.BeforeAction)
                    {
                        if (silinecekler.Count > 0)
                        {
                            foreach (var item in silinecekler)
                            {
                                if (item != "")
                                {
                                    ConstVariables.oRecordset.DoQuery("Delete from \"@AIF_ANLZGIRISBOLUM\" where \"DocEntry\" = '" + item + "'");
                                }
                            }
                        }
                    ...
                    //        //frmAnalizGirisBolumler.DataSources.DBDataSources.Item("@AIF_ANLZGIRISBOLUM").Clear();

                    //        //oMatrix.AddRow();
                    //        //frmAnalizGirisBolumler.Mode = BoFormMode.fm_UPDATE_MODE; 
                    //    }
                    //    catch (Exception ex)
                    //    {
                    //        Handler.SAPApplication.MessageBox("Hata oluştu" + ex.Message);
                ...
                case BoEventTypes.et_CLICK:
                    if (pVal.ItemUID == "1" && !pVal.BeforeAction)
                    {
                        ConstVariables.oRecordset.DoQuery("Select MAX(\"DocEntry\") from \"@AIF_ANLZGIRISBOLUM\"");
            ...
            try
            {
                if (pVal.MenuUID == "AIFRGHTCLK_DeleteRow" && pVal.BeforeAction)
                {
                    ...
                    if (row != -1)
                    {
                        ...
                        if (frmAnalizGirisBolumler.Mode == BoFormMode.fm_OK_MODE)
                        {
                            frmAnalizGirisBolumler.Mode = BoFormMode.fm_UPDATE_MODE;
                        }
                    }
